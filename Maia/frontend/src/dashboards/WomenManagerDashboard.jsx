...
function ProductsTab({ categories }) {
  ...
  const openEdit = (p) => {
    ...
    setForm({
      ...
      categoryName: p.category ?? '', color: p.color ?? ''
    })
        ...
        <Modal title={modal === 'add' ? 'Add Product' : 'Edit Product'} onClose={() => setModal(null)}
          ...
          <div className="db-form">
            <div className="db-field">
              ...
              <input className="db-input" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Summer Dress" />
            </div>
            <div className="db-form-row">
              ...
              <div className="db-field">
                ...
                <input className="db-input" list="women-cats" value={form.categoryName} onChange={e => setForm(f => ({ ...f, categoryName: e.target.value }))} placeholder="e.g. Dresses, Tops…" />
                <datalist id="women-cats">{cats.map(c => <option key={c.id} value={c.name} />)}</datalist>
              </div>
            </div>
            <div className="db-field">
            ...
            <div className="db-field">
              ...
              <input className="db-input" value={form.color} onChange={e => setForm(f => ({ ...f, color: e.target.value }))} placeholder="e.g. Red, Black (optional)" />
            </div>
            <div className="db-field">
        ...
        <Modal title={modal === 'add' ? 'Add Category' : 'Edit Category'} onClose={() => setModal(null)}
          ...
          <div className="db-form">
            <div className="db-field">
              ...
              <input className="db-input" value={form.name} onChange={e => setForm({ name: e.target.value })} autoFocus placeholder="e.g. Dresses" />
            </div>
          </div>
        </Modal>
      )}
...
function SalesTab() {
  ...
  const removeSale = async (p) => {
    try { await api.patch(`/CardsWomen/${p.id}/sale`, { discountPercent: 0 }); reload() }
...
export default function WomenManagerDashboard() {
  ...
  const { data: categories } = useApi(async () => { const r = await api.get('/WomanCategory'); return r.data }, [])
    ...
    <div className="db-root">
      ...
      <main className="db-main">
        <header className="db-header">
          <h1 className="db-header-title">{TABS.find(t => t.key === activeTab)?.label ?? 'Women Manager'}</h1>
        </header>
