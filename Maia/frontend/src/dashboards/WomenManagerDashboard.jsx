...
function ProductsTab({ categories }) {
  ...
  const del = async (p) => {
    ...
    try {
      await api.delete(`/CardsWomen/${p.id}`)
      reload()
    } catch (e) {
      const d = e?.response?.data
          ...
          <div className="db-form">
            ...
            <div className="db-field">
              ...
              <input
                ...
                placeholder="e.g. Red, Black (optional)"
              />
            </div>
            <div className="db-field">
...
export default function WomenManagerDashboard() {
  ...
  const { data: products } = useApi(
    async () => { const r = await api.get('/CardsWomen'); return r.data },
    []
  )
  const { data: categories } = useApi(
    async () => { const r = await api.get('/WomanCategory'); return r.data },
    []
  )
